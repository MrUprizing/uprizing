...
export default function ServicesUI({
  className,
  animatedBoxText = "Container",
  animatedText = "My pipeline...",
  appName = "Live App",
  circleText = "CI/CD",
  diamondText = "Backend",
  hintText = "(automated pipeline)",
  triangleText = "Cloud",
}: ShadCnUIProps) {
  return (
    <div className={cn("relative text-muted w-full aspect-[2/1]", className)}>
      {/* SVG */}
      <svg
        width="100%"
        height="100%"
      ...
      >
        ...
        {/* Left-Hand Shapes */}
        <g className="stroke-primary" strokeWidth="0.2" fill="none">
          {/* Registry Circle */}
          <circle
            cx="23"
            cy="14"
      ...
      <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
        {/* Shadcn TextBox */}
        <div className="relative flex justify-center rounded-md border bg-card px-4 py-1 text-sm shadow-sm">
          <p className="absolute -bottom-5 text-[10px] text-muted-foreground">
            {hintText}
          </p>
          <TextShimmer>{animatedText}</TextShimmer>
        </div>
      </div>
    </div>
  );
